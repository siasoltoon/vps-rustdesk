...
if ($installer.ExitCode -ne 0) { if (Test-Path $log) { Get-Content $log -Tail 80 }; throw "RustDesk MSI installation failed with exit code $($installer.ExitCode)." }
...
for ($attempt = 1; $attempt -le 4; $attempt++) { $service = Get-Service -Name $service.Name; if ($service.Status -eq "Running") { break }; try { Start-Service -Name $service.Name -ErrorAction Stop } catch { Write-Host "Service start attempt $attempt failed: $($_.Exception.Message)" }; Start-Sleep -Seconds 5 }
